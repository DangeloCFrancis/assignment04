...
rok_grdp <-
  read_excel("data/Korean GRDP Wikipedia Excel import_adj.xlsx") |>
  clean_names() |>
  ...
  rename(grdp_us_billion = gdp_us_billion)
...
korea_grdp_plot <-
  ggplot() + 
  geom_sf(data = rok_grdp_shapefile, aes(fill = grdp_us_billion)) +
  geom_sf(data = rok_major_cities) + # gives us city points 
  geom_label_repel(data = rok_major_cities, mapping = aes( x = lon, y = lat, label = name)) +
  scale_fill_gradient(
