...
rok_fertility_shapefile_metropolitan <-
  rok_fertility_shapefile |>
...
korea_fertility_plot <-
  ...
  geom_sf_text(data = rok_fertility_shapefile_metropolitan, mapping = aes(label = shapeName), label.size = 0.01, stat = "sf_coordinates",check_overlap = TRUE, size = 2.5, color = "black", position = position_dodge(),) +
...
rok_grdp$gdp_won <- 
 as.numeric(gsub("KR₩|trillion", "", rok_grdp$gdp_won))
...
korea_grdp_plot <-
  ggplot() + 
  geom_sf(data = rok_gdp_shapefile, aes(fill = gdp_us)) +
  scale_fill_gradient(
